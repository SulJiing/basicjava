...
public class MemberService {
   /*
    * ¾ÕÀ¸·Î ±¸¼ºÇØ¾ß ÇÒ °Í 1. ¾ÆÀÌµğ¿Í ºñ¹Ğ¹øÈ£¿¡ (¿µ¹®°ú ¼ıÀÚ Æ¯¼ö ±âÈ£) ÀÌ¿Ü (¶ç¾î¾²±â ¹× ÇÑ±Û)ÀÌ µé¾î°¡¸é °É·¯ÁÖ´Â ÄÚµå ÇÊ¿ä 2.
    * SQL¿¡ MEMBER ÀÇ °¢ ÄÃ·³ LENGTH¿¡ Á¤È®ÇÑ °ª ÇÊ¿ä È¤Àº ÄÚµå·Î ±æÀÌ¸¦ Á¤È®ÇÏ°Ô ³ª´©¾îÁÖ¾î¾ßÇÔ. 3.
    */

   static public Map<String, Object> loginMap = new HashMap<>();
   // SIGN UP È­¸é´Ü Ç¥½Ã¸¦ À§ÇÑ LIST
   ...
   public int login() {
      ...
      // ¾ÆÀÌµğ length ¿¹¿ÜÃ³¸®
      if (id.length() < 4 || id.length() >= 7 || id.matches(".*[°¡-ÆR]+.*") || id.contains(" ")){
      ...
      // ºñ¹Ğ¹øÈ£ length ¿¹¿ÜÃ³¸®
      if (password.length() < 4 || password.length() >= 7 || password.matches(".*[°¡-ÆR]+.*") || password.contains(" ")) {
   ...
   public int infoUpdate() {
      ...
      if (Controller.sessionStorage.get("login") == (Object) false) {
         ...
         if (flag.equalsIgnoreCase("y")) {
            ...
            if (pass.length() < 4 || pass.length() >= 7|| pass.matches(".*[°¡-ÆR]+.*") || pass.contains(" ")) {
               System.out.println("ºñ¹Ğ¹øÈ£ Çü½ÄÀ» È®ÀÎÇØÁÖ¼¼¿ä.");
               return infoUpdate();
            }
